...
def CommandProc(stt):
    ...
    for i in range(len(cmdLists)):
        if cmdLists[i][0] in str(cmd):
            ...
            len_of_answer = len(cmdLists[i][1]) # 대답하는 글자 수
            print("len_of_answer:",len_of_answer)
            motion_repeat_num = int(round(len_of_answer/20))-2 # 모션 반복 횟수 
            speak(cmdLists[i][1], len(cmdLists[i][1])/5, global_vol)

            print("\n>>말해주세요(일상 또는 대화)~")
            print("cmdLists[i][3] : ", cmdLists[i][3])
            gsp.resumeMic()
            # print("gsp : ", gsp.status)
            return cmdLists[i][2]
    # 리스트에 없는 명령어일 경우 
...
def eye_tracking(r_arm, r_hand, motionData_x, motionData_y, l_arm, l_hand):
    ...
    # elif status_speak_mode == 3 # 안내모드에서 안내 끝나고 질문할 때 동작
    #     motion_once += 1
    #     m.set_motion(name="wake_up3", cycle=1)
    #     motion_once = 0
    #     status_speak_mode = 2
